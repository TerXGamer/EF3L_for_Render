...
function compactCloudState(snapshot) {
  const compact = normalizeState(snapshot);
  ...
  compact.instances = Object.fromEntries(
    Object.entries(compact.instances).map(([id, item]) => [
      id,
      removeEmptyFields({
        id: item.id,
        taskId: item.taskId,
        ...
        requiredOverdue: item.requiredOverdue || undefined,
        importance: item.importance,
        status: item.status,
        completedAt: item.completedAt,
        completedFrom: item.completedFrom,
...
function statsForDates(dates, filter = null) {
  ...
  return { total, completed, missed, rate, items };
}
...
function renderAuditStats() {
  ...
  const task = selectedAuditTaskId ? getTask(selectedAuditTaskId) : null;
  if (!task) {
  ...
  renderAuditTaskStats(task);
}

function renderAuditTaskOptions() {
  if (!el.auditTaskSelect) return;
  const tasks = state.tasks.slice().sort(sortTasks);
  if (selectedAuditTaskId && !tasks.some((task) => task.id === selectedAuditTaskId)) {
    selectedAuditTaskId = "";
    auditStatsMode = "";
  }
  el.auditTaskSelect.innerHTML = [`<option value="">اختر مهمة</option>`]
    .concat(tasks.map((task) => `<option value="${escapeHtml(task.id)}">${escapeHtml(task.title)}</option>`))
    .join("");
  el.auditTaskSelect.value = selectedAuditTaskId;
  el.auditTaskSelect.disabled = tasks.length === 0;
}
...
function renderAuditTaskStats(task) {
  const today = todayISO();
  const taskKey = auditTaskKey(task.title);
  const taskFilter = (item) => item.taskId === task.id || auditTaskKey(item.title) === taskKey;
  const weekDates = rangeDates(startOfWeek(today), today);
  const monthDates = rangeDates(startOfMonth(today), today);
...
function auditMonthlyTaskSummaries() {
  ...
  return Array.from(groups.values())
    .map(({ title, items }) => {
      ...
      const missedRate = total ? Math.max(0, 100 - completedRate) : 0;
      return {
        title,
        total,
        completed,
